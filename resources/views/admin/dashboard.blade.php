<x-layout>
    ...
    <div class="flex flex-col md:flex-row gap-5 mb-5">
        <div class="max-w-screen-lg mx-auto w-full overflow-x-hidden flex flex-col gap-5">
            ...
            <div id="listMahasiswa" class="p-4 max-h-[360px] overflow-y-auto space-y-3 text-sm text-gray-700 dark:text-gray-200">
                ...
                @else
                    <div class="text-center text-gray-500 dark:text-gray-300 italic">
                        📋 Belum ada data mahasiswa yang hadir hari ini.
                    </div>
                @endif
            </div>
    ...
    <div class="flex flex-col md:flex-row gap-5 mb-5 w-full">
        <div class="mx-auto w-full flex flex-col md:flex-row gap-5">
            ...
            <div class="w-full md:w-3/4 bg-white dark:bg-gray-800 rounded-md shadow-xl">
                ...
                <div class="overflow-x-auto w-[310px] sm:w-150 md:w-full mt-3 pb-3">
                ...
                <div class="p-4 space-y-4 text-sm text-gray-700 dark:text-gray-200 max-h-[430px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
                    @if (count($tidakHadir) > 0)
                        @foreach ($tidakHadir as $index => $th)
                            <div x-show="showAll || {{ $index }} < 10" x-transition
                                class="item-mahasiswa flex items-start gap-4 p-3 rounded-md border border-red-200 dark:border-gray-600 hover:shadow transition-all">
                                <img src="https://ui-avatars.com/api/?name={{ urlencode($th->mahasiswa->nama) }}&background=EF4444&color=fff"
                                    alt="{{ $th->mahasiswa->nama }}"
                        ...
                        @else
                    <div class="text-center text-gray-500 dark:text-gray-300 italic">
                        📭 Belum ada data mahasiswa yang tidak hadir hari ini.
                    </div>
                @endif
                </div>
...
{{-- @push('scripts')
    <script>
        const chartData = @json($mingguan); // hanya muncul kalau $mingguan tersedia
    </script>
    <script src="{{ asset('js/chart') }}"></script>
@endpush --}}
