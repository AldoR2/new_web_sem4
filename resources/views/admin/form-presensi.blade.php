<x-layout>
    ...
    <div class="h-full dark:text-white">
        ...
        <div class="w-full h-max max-w-full mt-5 p-8 bg-white rounded-sm shadow-xl dark:bg-gray-800">
            ...
            <form action="{{ isset($presensi) ? route('admin.presensi.update', $presensi->id) : route('admin.presensi.store') }}" method="POST" class="form-validasi">
                ...
                <div x-data="{ status: '{{ old('status', $presensi->pertemuan->status ?? '') }}' }">
                    ...
                    {{-- <div class="flex flex-col md:flex-row" x-show="status && status !== 'libur'" x-transition x-cloak> --}}
                        ...
                        {{-- <div class="flex flex-col w-full mb-4 md:w-1/3 mr-0 md:mr-4">
                            ...
                            <input type="date" id="tgl_presensi" name="tgl_presensi" class="p-2 border-2 mt-1 border-gray-400 dark:border-gray-600 dark:bg-gray-600 dark:text-white rounded-sm" value="{{old('tgl_presensi', $presensi->tgl_presensi ?? '')}}" placeholder="Masukkan tanggal presensi">
                        ...
                        <div class="flex flex-col w-full mb-4 md:w-1/3 mr-0 md:mr-4">
                            ...
                            <input type="time" id="jam_awal" name="jam_awal" value="{{old('jam_awal', $presensi->jam_awal ?? '')}}" class="p-2 w-full border-2 border-gray-400 rounded-sm dark:bg-gray-600 dark:border-gray-600 dark:text-white" placeholder="Masukkan Jam Awal">
                        ...
                        <div class="flex flex-col w-full mb-4 md:w-1/3">
                            ...
                            <input type="time" id="jam_akhir" name="jam_akhir" value="{{old('jam_akhir', $presensi->jam_akhir ?? '')}}" class="p-2 w-full border-2 border-gray-400 dark:border-gray-600 rounded-sm dark:bg-gray-600 dark:text-white" placeholder="Masukkan Jam Akhir">
                ...
                {{-- <div x-data="{
                    jumlah: {{count(old('tgl_presensi', $presensi->tgl_presensi ?? [1])) }},
                    status: '{{ old('status', $presensi->pertemuan->status ?? '') }}'
                }"> --}}

                {{-- <div x-data="{
                    jumlah: {{count(old('tgl_presensi', $presensi->tgl_presensi ?? [1])) }},
                    presensis: {{ Js::from(old('presensi', [
                        ['tgl_presensi' => $presensi->tgl_presensi ?? '',
                        'jam_awal'     => $presensi->jam_awal ?? '',
                        'jam_akhir'    => $presensi->jam_akhir ?? '']
                    ])) }}
                }"> --}}

                {{-- <div x-data="{ --}}
                    {{-- jumlah: {{ count(old('tgl_presensi', $presensi->tgl_presensi ? [$presensi->tgl_presensi] : [])) ?: 1 }}, --}}
                    {{-- jumlah: {{count(old('tgl_presensi', $presensi->tgl_presensi ?? [1])) }}, --}}
                    {{-- presensis: {{ Js::from(old('presensi', [
                        ['tgl_presensi' => $presensi->tgl_presensi ?? '',
                        'jam_awal'     => $presensi->jam_awal ?? '',
    ...
    {{-- <div class="flex flex-col w-full mb-4 md:w-1/2">
        <label for="jumlah" class="mb-1 font-semibold dark:text-white">Jumlah Pertemuan:</label>
        <input type="number" id="jumlah" min="1" x-model.number="jumlah"
        ...
        {{-- <div class="flex flex-col md:flex-row">
            <div class="flex flex-col w-full mb-4 md:w-1/2 mr-0 md:mr-8">
                <label :for="'pertemuan_' + i" class="mb-1 font-semibold dark:text-white">Pertemuan Ke :</label>
                <select :id="'pertemuan_' + i" :name="'pertemuan_ke_' + i" class="p-2 w-full border-2 border-gray-400 dark:border-gray-600 dark:bg-gray-600 dark:text-white rounded-sm" required>
                    <option value="" hidden selected>Pilih Pertemuan</option>
                        @for($i = 1; $i <= 50; $i++)
                            <option value="{{ $i }}" @if (old('pertemuan_ke', $presensi->pertemuan->pertemuan_ke ?? '') == $i) selected @endif>
                                {{$i}}
            ...
            <div class="flex flex-col w-full mb-4 md:w-1/2">
                <label :for="'jenis_' + i" class="mb-1 font-semibold dark:text-white">Jenis Perkuliahan:</label>
                <select :id="'jenis_' + i" :name="'jenis['+i+']'"
                    class="p-2 mt-1 w-full border-2 border-gray-400 dark:border-gray-600 dark:bg-gray-600 dark:text-white rounded-sm"
                    x-bind:disabled="status !== 'aktif'">
                    <option value="" hidden selected>Pilih Jenis Perkuliahan</option>
                    <option value="teori" {{ old('jenis', $presensi->pertemuan->jenis ?? '') == 'teori' ? 'selected' : '' }}>Teori</option>
                    <option value="praktik" {{ old('jenis', $presensi->pertemuan->jenis ?? '') == 'praktik' ? 'selected' : '' }}>Praktik</option>
                </select>
        ...
        {{-- <div class="md:flex-row mb-4 p-3 border rounded-md dark:border-gray-600">
            ...
            <div class="flex flex-col md:flex-row w-full mb-4">
                <div class="flex flex-col w-full mb-4 md:w-1/2 mr-0 md:mr-8">
                    <label :for="'pertemuan'" class="mb-1 font-semibold dark:text-white">Pertemuan Ke :</label>
                    <select :id="'pertemuan'" :name="'pertemuan_ke[]'" x-model="pertemuan_ke" class="p-2 w-full border-2 border-gray-400 dark:border-gray-600 dark:bg-gray-600 dark:text-white rounded-sm" required>
                        <option value="" hidden selected>Pilih Pertemuan</option>
                            @for($i = 1; $i <= 50; $i++)
                                <option value="{{ $i }}" @if (old('pertemuan_ke.'. $i, $presensi->pertemuan->pertemuan_ke ?? '') == $i) selected @endif>
                                    {{$i}}
                                </option>
                            @endfor
                    </select>
                ...
                <div class="flex flex-col w-full mb-4 md:w-1/2">
                    <label :for="'jenis'" class="mb-1 font-semibold dark:text-white">Jenis Perkuliahan:</label>
                    <select :id="'jenis'" :name="'jenis[]'" x-model="jenis" class="p-2 mt-1 w-full border-2 border-gray-400 dark:border-gray-600 dark:bg-gray-600 dark:text-white rounded-sm">
                        <option value="" hidden selected>Pilih Jenis Perkuliahan</option>
                        <option value="teori" {{ old('jenis.'. $i, $presensi->pertemuan->jenis ?? '') == 'teori' ? 'selected' : '' }}>Teori</option>
                        <option value="praktik" {{ old('jenis.'. $i, $presensi->pertemuan->jenis ?? '') == 'praktik' ? 'selected' : '' }}>Praktik</option>
                    </select>
            ...
            {{-- <div class="flex flex-col md:flex-row w-full">
                <div class="flex flex-col w-full mb-2 md:w-1/3 mr-0 md:mr-4">
                    <label :for="'tgl_presensi_' + i" class="mb-1 font-semibold dark:text-white">Tanggal (@{{ i }})</label>
                    <input type="date" :id="'tgl_presensi_' + i" :name="'tgl_presensi[]'"
                     x-model="tgl_presensi"
                ...
                <div class="flex flex-col w-full mb-2 md:w-1/3 mr-0 md:mr-4">
                    <label :for="'jam_awal_' + i" class="mb-2 font-semibold dark:text-white">Jam Mulai (@{{ i }})</label>
                    <input type="time" :id="'jam_awal_' + i" :name="'jam_awal[]'"
                    x-model="jam_awal"
                        class="p-2 border-2 border-gray-400 dark:border-gray-600 dark:bg-gray-600 dark:text-white rounded-sm">
                    <span class="text-red-600 text-sm" id="jam_awal_error">
                        @error('jam_awal.$i'){{ $message }}@enderror
                    </span>
                </div>

                <div class="flex flex-col w-full mb-2 md:w-1/3">
                    <label :for="'jam_akhir_' + i" class="mb-2 font-semibold dark:text-white">Jam Selesai (@{{ i }})</label>
                    <input type="time" :id="'jam_akhir_' + i" :name="'jam_akhir['+i+'\]'"
                    x-model="jam_akhir"
                        class="p-2 border-2 border-gray-400 dark:border-gray-600 dark:bg-gray-600 dark:text-white rounded-sm">
                    <span class="text-red-600 text-sm" id="jam_akhir_error">
                        @error('jam_akhir.$i'){{ $message }}@enderror
                    </span>
                </div>
            </div>
        </div>
    ...
    @if (!isset($presensi))
        <div
            ...
            <div class="mt-4" x-show="status && status === 'aktif'" x-transition x-cloak>
    ...
    @if (isset($presensi))
        <div
            x-data="{
                pertemuan_ke : '{{ $presensi->pertemuan->pertemuan_ke ?? '' }}',
                jenis : '{{ $presensi->pertemuan->jenis ?? '' }}',
                tgl_presensi : '{{ $presensi->tgl_presensi ?? '' }}',
                jam_awal : '{{ $presensi->jam_awal ?? '' }}',
                jam_akhir : '{{ $presensi->jam_akhir ?? '' }}'
            }">
...
document.addEventListener('alpine:init', () => {
    Alpine.data('presensiForm', () => ({
        jumlah: 1,
        presensis: [{}],

        init() {
            // Inisialisasi data dari server jika ada
            this.$watch('jumlah', (value) => {
                const currentLength = this.presensis.length;
